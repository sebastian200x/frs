...
<div class="content bg-gradient-orange py-3 px-4">
                        ...
                        <div id="menu-list">
                            <fieldset>
                                ...
                                <div id="cat-list" class="py-1">
                                    <?php 
                                    $cid='';
                                    $categories = $conn->query("SELECT * FROM `tblcategorylist` where  delete_flag = 0 and `status` = 1 order by `name` asc");
                                    while($row = $categories->fetch_assoc()):
                                        if(empty($cid)){
                                            $cid = $row['category_id'];
                                        }
                                    ?>
                                    <button class="btn btn-default btn-xs rounded-pill px-2 cat_btn mx-3 col-lg-3 col-md-4 col-sm-6 col-xs-10 <?= isset($cid) && $cid == $row['category_id'] ? "bg-gradient-orange text-light" : "bg-gradient-light border" ?>" type="button" data-id='<?= $row['category_id'] ?>'><?=  $row['name'] ?></button>
                                    <?php endwhile; ?>
                                </div>
                            </fieldset>
                            <fieldset>
                                ...
                                <div id="item-list" class="py-1 overflow-auto">
                                    ...
                                    <?php 
                                    $conn->query("UPDATE tblmenulist m JOIN tblstocks s on m.menu_id = s.menu_id SET m.status = 0 WHERE s.amount <= 0");
                                    $menus = $conn->query("SELECT * FROM `tblmenulist` where  delete_flag = 0 and `status` = 1 order by `name` asc");
                                    while($row = $menus->fetch_assoc()):
                                        
                                    ?>
                                    <div class="col <?= isset($cid) && $cid == $row['category_id'] ? "" : "d-none" ?> menu-item" data-cat-id='<?= $row['category_id'] ?>'>
                                        <button class="btn btn-default btn-block btn-xs rounded-pill px-2 bg-gradient-light border my item-btn" type="button" data-id='<?= $row['menu_id'] ?>' data-price='<?= $row['price'] ?>'><p class="m-0 truncate-1"><?= $row['code'] . " - " . $row['name'] ?></p></button>
                                    </div>
                                    <?php endwhile; ?>
                                    </div>
                                </div>
                            ...
                            <div class="text-center py-2">
                                <button class="btn btn-orange bg-gradient-orange rounded-pill px-4">Place Order</button>
                            </div>
                        </div>
                        <div id="order-list" class="bg-gradient-dark p-1">
                            ...
                            <div id="order-items-holder" class="bg-gradient-light mb-3">
                                <div id="order-items-header">
                                    <div class="d-flex w-100 bg-gradient-orange">
...
<div class="d-flex w-100 bg-gradient-light product-item">
    <div class="col-3 text-center font-weight-bolder m-0 border align-middle">
        ...
        <div class="input-group input-group-sm">
            <button class="btn btn-orange btn-xs btn-flat minus-qty" type="button"><i class="fa fa-minus"></i></button>
            <input type="number" min = '1' value='1' name="quantity[]" class="form-control form-control-xs rounded-0 text-center" required readonly>
            <button class="btn btn-orange btn-xs btn-flat plus-qty" type="button"><i class="fa fa-plus"></i></button>
        </div>
    </div>
...
<script>
    ...
    $(function(){
        ...
        $('.cat_btn').click(function(){
            $('.cat_btn.bg-gradient-orange').removeClass('bg-gradient-orange text-light').addClass('bg-gradient-light border')
            $(this).removeClass('bg-gradient-light border').addClass('bg-gradient-orange text-light')
