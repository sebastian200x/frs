...
function disableButtonWithCooldown() {
    $('#user_list_btn').prop('disabled', true); // Disable the button
    loginAttempts++; // Increment login attempts

    // Check if maximum login attempts reached
    if (loginAttempts >= 3) {
        // Set a cooldown for 3 seconds before enabling the button again
        setTimeout(function() {
            $('#user_list_btn').prop('disabled', false); // Enable the button
